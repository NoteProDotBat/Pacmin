...
if %errorlevel%==5 call :Load 5 "play"
goto:menu
...
set lastDirection=-1
set nextDirextion=0
set "up=0" && set "down=1" && set "left=2" && set "right=3"
mode 91, 33
call "Pacminboard.bat"
:Game
title Pacmin %version% x, %x% y, %y%  %up%,%down%,%left%,%right%

:: Sprites for different directions (compressed into one echo)
:: redundant drawing reduced for moving the same direction
if %direction% NEQ %lastDirection% (
...
set /a x2 = %x% + 57
:: draw the board around the player
...
:: Key up pressed
if %errorlevel%==294 set direction=0
:: Key down pressed				175
if %errorlevel%==296 set direction=1
:: Key left pressed
if %errorlevel%==293 set direction=2
:: Key right pressed
if %errorlevel%==295 set direction=3

if %direction%==0 if !y! GTR -4 (set/a y-=2) ELSE (set y=-4)
if %direction%==1 if !y! LSS 185 (set/a y+=2) ELSE (set y=186)
if %direction%==2 if !x! GTR 0 set/a x-=4
if %direction%==3 if !x! LSS 360 set/a x+=4
